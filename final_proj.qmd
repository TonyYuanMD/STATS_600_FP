...
Notice that $ALQ111$, as a factor variable, has only one level, so it
may cause problems when fitting models.

From the plot above we can see that the problem of imbalanced data
exists, as the proportion of depressed cases is much lower than that of
its counterpart. To address this problem we propose several approaches:
firstly, we use stratified sampling instead of random data splitting;
secondly, we assigned higher weights to the depressed cases in training.
Notice that $ALQ111$, as a factor variable, has only one level, so it
may cause problems when fitting models.
...
Here we fit the vanilla logistic regression model using all the
variables naively, except the $ALQ111$.
...
The above is fitted with only one fold of training data, so here we try
a 5-fold cross validation on the logistic regression.
...
In the last section, we used all the random variables, trying to improve
the prediction ability of the model. Here we want to check if the
selected variables will be different by first inspecting correlations
between each variables and the response, and draw inference from the
logistic regression fitted on the subset of variables.
...
If we convert unordered variables to factor type and keep ordered
categorical variables as numeric type, the output of applying LASSO
follows.
...
```
Find the best lambda, choose lambda min as the best lambda.
```{r}
...
selected_features %>%
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "#DB6565") +
  labs(
    title = "LASSO Selected Variables and Coefficients (MIN)",
    x = "Variable",
    y = "Coefficient"
  ) +
  theme_minimal()+
...
ggplot(roc_data, aes(x = FPR, y = TPR)) +
  ...
  annotate("text", x = 0.75, y = 0.25, label = paste("AUC =", round(auc_value, 2)), 
           color = "#DB6565", size = 5, fontface = "bold") +  # Add AUC annotation
  labs(
    title = "Receiver Operating Characteristic (ROC) Curve (MIN)",  # Title
    x = "False Positive Rate (1 - Specificity)",              # X-axis label
    y = "True Positive Rate (Sensitivity)"                   # Y-axis label
  ) +
  theme_minimal() +  # Minimal theme for clean appearance
...
### Choose lambda 1se as the best lambda.
...
selected_features %>%
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "#DB6565") +
  labs(
    title = "LASSO Selected Variables and Coefficients (1SE)",
    x = "Variable",
    y = "Coefficient"
  ) +
  theme_minimal()+
...
ggplot(roc_data, aes(x = FPR, y = TPR)) +
  ...
  annotate("text", x = 0.75, y = 0.25, label = paste("AUC =", round(auc_value, 2)), 
           color = "#DB6565", size = 5, fontface = "bold") +  # Add AUC annotation
  labs(
    title = "Receiver Operating Characteristic (ROC) Curven (1SE)",  # Title
    x = "False Positive Rate (1 - Specificity)",              # X-axis label
    y = "True Positive Rate (Sensitivity)"                   # Y-axis label
  ) +
  theme_minimal() +  # Minimal theme for clean appearance
...
# Nested Model Random Effects
ggplot(df_nest, aes(x = Group, y = Estimate)) +
  geom_bar(stat = "identity", fill = "#DB6565", color = "#5C5C5C", width = 0.7) +
  labs(
    title = "Random Effects (Nested Model)",
    x = "Race:Marital Status",
    y = "Random Effect"
  ) +
...
ggplot(df_cross_race, aes(x = Group, y = Estimate)) +
  geom_bar(stat = "identity", fill = "#EB8E55", color = "#5C5C5C", width = 0.7) +
  labs(
    title = "Random Effects (Crossed Model - Race)",
    x = "Race",
    y = "Random Effect"
  ) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +  # Reference line at 0
...
# Crossed Model (Marital Status)
ggplot(df_cross_marital, aes(x = Group, y = Estimate)) +
  geom_bar(stat = "identity", fill = "#4E73E3", color = "#5C5C5C", width = 0.7) +
  labs(
    title = "Random Effects (Crossed Model - Marital Status)",
    x = "Marital Status",
    y = "Random Effect"
  ) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +  # Reference line at 0
