...
continuous_mapping <- data.frame(
  DummyVariable = selected_dummy_variables[selected_dummy_variables %in% continuous_vars],
  OriginalVariable = selected_dummy_variables[selected_dummy_variables %in% continuous_vars]
)
...
#Note that we forget ALQ151, ALQ121. Compared with the selected features above.
selected_original_vars <- c(selected_original_vars, "ALQ121", "ALQ151")
...
# Fit the random effects model
me_int <- glmer(
  depressed ~ DMDEDUC2 + INDFMPIR + BMXBMI + INDFMMPC + ALQ151 + BPQ020 + BPQ080 + DBQ700 + DBD910 + SMQ020 + DIQ010 + MCQ010 + MCQ053 + RIDRETH1 * DMDMARTZ + (1 | RIDRETH1 / DMDMARTZ) + (1 | DMDBORN4) + (1 | RIAGENDR),
  data = train_data,
  family = binomial(link = "logit"),
  weights = ifelse(train_data$depressed == "Yes", 10, 1)
)
...
anova(me, me_int)
emmeans_results <- emmeans(me_interaction, ~ DMDMARTZ | RIDRETH1)
