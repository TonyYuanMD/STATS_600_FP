...
Please make sure the following library are correctly installed on your machine, so you can run through the whole markdown file all at once.
...
From the plot above we can see that the problem of imbalanced data exists, as the proportion of depressed cases is much lower than that of its counterpart. To address this problem we propose several approaches:
...
The above is fitted with only one fold of training data, so here we try a 5-fold cross validation on the logistic regression.
...
In the last section, we used all the random variables, trying to improve he prediction ability of the model. Here we want to check if the selected variables will be different by first inspecting correlations between each variables and the response, and draw inference from the logistic regression fitted on the subset of variables.
